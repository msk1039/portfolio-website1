document.addEventListener("astro:before-swap", (e) =>
  [...e.newDocument.head.querySelectorAll('link[as="font"]')].forEach((t) =>
    t.remove()
  )
);
...
async function ie(e, t, n, o, r, a, u, d) {
  ...
  return (
    document.dispatchEvent(s) &&
      (await s.loader(),
      s.defaultPrevented ||
        (oe(ee), s.navigationType !== "traverse" && k({ scrollX, scrollY }))),
    s
  );
}
...
const K = (e, t, n, o, r) => {
  ...
  if (e.href !== location.href && !r)
    if (n.history === "replace") {
      ...
      S(
        { ...n.state, index: s.index, scrollX: s.scrollX, scrollY: s.scrollY },
        "",
        e.href
      );
  ...
  if (
    ((P = e),
    a || (scrollTo({ left: 0, top: 0, behavior: "instant" }), (d = !0)),
    r)
  )
    ...
    if (e.hash) {
      ...
      const s = history.state;
      (location.href = e.href), history.state || S(s, "");
  ...
  const r = (i, l) => {
    ...
    s = (i) => {
      const l = document.documentElement,
        h = [...l.attributes].filter(
          ({ name: c }) => (l.removeAttribute(c), c.startsWith("data-astro-"))
        );
      [...i.newDocument.documentElement.attributes, ...h].forEach(
        ({ name: c, value: f }) => l.setAttribute(c, f)
      );
      for (const c of document.scripts)
        for (const f of i.newDocument.scripts)
          if (
            !f.hasAttribute("data-astro-rerun") &&
            ((!c.src && c.textContent === f.textContent) ||
              (c.src && c.type === f.type && c.src === f.src))
          ) {
            f.dataset.astroExec = "";
            break;
          }
      ...
      for (const c of g.querySelectorAll(`[${p}]`)) {
        ...
        v &&
          ...
          v.localName === "astro-island" &&
            d(c) &&
            (c.setAttribute("ssr", ""),
            c.setAttribute("props", v.getAttribute("props"))));
      }
      u(T);
    };
  async function m(i) {
...
async function G(e, t, n, o, r) {
  ...
  if (((L = !1), I))
    y = document.startViewTransition(async () => await M(u, o, r));
  else {
    const s = (async () => {
...
async function F(e, t) {
  await G("forward", P, new URL(e, location.href), t ?? {});
}
function me(e) {
  if (!O() && e.state) {
    location.reload();
  ...
  const t = history.state,
    n = t.index,
    o = n > E ? "forward" : "back";
  (E = n), G(o, P, new URL(location.href), {}, t);
}
const X = () => {
  history.state &&
...
function we() {
  for (const e of ["touchstart", "mousedown"])
    document.body.addEventListener(
      ...
      (t) => {
        A(t.target, "tap") &&
          x(t.target.href, { with: "fetch", ignoreSlowConnection: !0 });
      },
      { passive: !0 }
    );
}
...
function ye() {
  ...
  H(() => {
    for (const t of document.getElementsByTagName("a"))
      D.has(t) || (A(t, "viewport") && (D.add(t), (e ??= be()), e.observe(t)));
  });
}
function be() {
  const e = new WeakMap();
